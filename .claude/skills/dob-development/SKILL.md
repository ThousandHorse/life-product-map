...
| 勤怠Pane3 月切替 | `<select>` プルダウン（今月〜過去12ヶ月）。左右矢印は廃止 | 直接目的月にジャンプできる方がUXが良いため |
| 勤怠Pane3 テンプレ | エクスポートボタン右隣に「テンプレ読み込み」ボタン。.csv/.xlsx/.xls 対応 | 既存勤務表からのインポートを想定。パース実装は Step 8/9 で対応 |
| NavPane 年目標配置 | 目標モード行の直下に年目標リスト、区切り線の下に勤怠モード行 | モード切替に関わらず常時表示し、目標への素早いアクセスを優先 |
