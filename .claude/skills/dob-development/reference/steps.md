...
条件は上から順に評価し、最初に一致したものを採用する（danger を最優先）。

| 優先度 | 条件 | ステータス |
|---|---|---|
| 1（最優先） | 達成率 < 40% かつ 残日数 < 7日 | "danger"（🔴 危険） |
| 2 | 達成率 < 40% または 残日数 < 7日 | "danger"（🔴 危険） |
| 3 | 達成率 40〜70% かつ 残日数 7〜14日 | "caution"（⚠️ 注意） |
| 4（該当なし） | 上記以外 | "ok"（✅ 順調） |
...
## Step 8: 勤怠モード - 月次一覧（AttendanceListPane）

**作成ファイル:**
- `components/workspace/AttendanceListPane.tsx`（Pane 3 の勤怠モード）

**実装内容:**
- ヘッダー左: 月タイトル + 月切替プルダウン（`<select>`）を横並び。今月〜過去12ヶ月を降順でリスト表示
- ヘッダー右: 「エクスポート」「テンプレ読み込み」ボタンを横並び
...
- **注意**: 休憩時間が勤務時間内に収まっているかの検証（重複チェック）と日跨ぎ勤務（例: 22:00〜02:00）への対応が必要。React 実装時は `date-fns` 等を使い、モックの単純計算をそのまま移植しないこと
- 打刻済みの行と未打刻の行をビジュアルで区別
- 「テンプレ読み込み」ボタンでファイル選択ダイアログを開く（.csv / .xlsx / .xls を受け付け）

**完了条件:**
- [ ] ヘッダー左に「月タイトル + 月切替プルダウン」が表示される
- [ ] ヘッダー右に「エクスポート」「テンプレ読み込み」ボタンが表示される
...
- [ ] 月切替プルダウンで月を変更できる（過去12ヶ月分）
- [ ] 「テンプレ読み込み」ボタンがファイル選択ダイアログを開く
...
**注意事項:**
- テンプレ読み込み（会社指定フォーマットの xlsx/csv インポート + 列マッピング）は Phase 2 以降のスコープ
  - Phase 1 では UI（ファイル選択ダイアログ）のみを実装し、実際のパース処理は行わない
- Phase 1 のエクスポートは汎用的な固定フォーマット（列順固定）で出力する
