...
- shadcn base-nova: `asChild` ではなく `render` prop を使っているか
- 色トークン: `bg-blue-500` 等の色番号直書きがないか
- 間隔管理: `space-y-*` を使っていないか（`flex gap-*` を使う）
- コメント: WHY を書いているか、WHAT だけのコメントがないか

### レビュー観点
1. **完了条件**: steps.md の完了条件を全て満たしているか
2. **ビルド**: `npm run build` でエラーがないか
3. **型チェック**: `npm run lint` でエラーがないか
4. **コードの一貫性**: 既存コンポーネントのスタイルと揃っているか
